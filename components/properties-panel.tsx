...
import { Trash2, FileCode, Copy, Check } from "lucide-react"
...
export function PropertiesPanel({
  ...
  if (!block) {
    ...
    const handleCopy = (fileName: string) => {
      navigator.clipboard.writeText(terraformFiles[fileName as keyof typeof terraformFiles])
              ...
              {Object.keys(terraformFiles).map((fileName) => (
                <button
                  ...
                  className={`w-full text-left px-3 py-2 rounded-md text-sm font-mono transition-colors ${
                    selectedFile === fileName ? "bg-primary/10 text-primary" : "hover:bg-muted/50 text-muted-foreground"
                  }`}
                >
                  📄 {fileName}
                </button>
          ...
          <ScrollArea className="flex-1">
            <div className="p-4">
              <div className="flex items-center justify-between mb-2">
                ...
                <Button size="sm" variant="ghost" className="h-7 text-xs" onClick={() => handleCopy(selectedFile)}>
          ...
          <div className="p-4 border-t border-sidebar-border">
            <div className="bg-muted/30 rounded-md p-3 text-xs text-muted-foreground">
              ...
              <ol className="list-decimal list-inside space-y-1">
                <li>Copy all files to your project</li>
                <li>
                  Run <code className="bg-background px-1 rounded">terraform init</code>
                </li>
                <li>
                  Run <code className="bg-background px-1 rounded">terraform plan</code>
                </li>
                <li>
                  Run <code className="bg-background px-1 rounded">terraform apply</code>
                </li>
              </ol>
            </div>
          </div>
